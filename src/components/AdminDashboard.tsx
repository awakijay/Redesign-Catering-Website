...
export function AdminDashboard() {
  ...
  const [newGallery, setNewGallery] = useState({ src: '', alt: '', category: 'Events' });
  ...
  const addProduct = () => {
    ...
    const payload: AdminProduct = { id: Date.now().toString(), ...newProduct };
  ...
  const addGalleryItem = () => {
    ...
    const payload: AdminGalleryImage = { id: Date.now().toString(), ...newGallery };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-blue-50">
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Catering Admin Panel</h1>
            <p className="text-sm text-slate-600">Manage website content, requests, and customer conversations.</p>
          </div>
          <button onClick={handleLogout} className="inline-flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-red-700">
            <LogOut className="h-4 w-4" /> Logout
          </button>
        </div>
      </header>

      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[250px_1fr] lg:px-8">
        <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="space-y-2">
            {navItems.map((item) => {
              ...
              return (
                <button key={item.key} onClick={() => setActiveTab(item.key)} className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition-colors ${isActive ? 'bg-gradient-to-r from-blue-600 to-cyan-500 text-white shadow' : 'text-slate-700 hover:bg-slate-100 hover:text-slate-900'}`}>
            ...
            <section className="space-y-6">
              <div className="overflow-hidden rounded-2xl bg-slate-900 text-white">
                <div className="grid grid-cols-1 lg:grid-cols-2">
                  ...
                  <img src="https://images.unsplash.com/photo-1556910103-1c02745aae4d?auto=format&fit=crop&w=1200&q=80" alt="Catering admin dashboard" className="h-full w-full object-cover" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {[
                  ...
                  <div key={label} className={`rounded-xl bg-gradient-to-r ${gradient} p-5 text-white shadow`}>
            ...
            <section className="space-y-6">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <input value={newProduct.name} onChange={(event) => setNewProduct((old) => ({ ...old, name: event.target.value }))} placeholder="Product name" className={inputClass} />
                  <input value={newProduct.price} onChange={(event) => setNewProduct((old) => ({ ...old, price: event.target.value }))} placeholder="Price" className={inputClass} />
                  <input value={newProduct.category} onChange={(event) => setNewProduct((old) => ({ ...old, category: event.target.value }))} placeholder="Category" className={inputClass} />
                  <input value={newProduct.image} onChange={(event) => setNewProduct((old) => ({ ...old, image: event.target.value }))} placeholder="Image URL (optional)" className={inputClass} />
                  <textarea value={newProduct.description} onChange={(event) => setNewProduct((old) => ({ ...old, description: event.target.value }))} placeholder="Description" rows={3} className={`${inputClass} md:col-span-2`} />
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-4">
                  <label className="inline-flex items-center gap-2 text-sm text-slate-700"><input type="checkbox" checked={newProduct.isNew} onChange={(event) => setNewProduct((old) => ({ ...old, isNew: event.target.checked }))} /> Mark as new</label>
                  <button onClick={addProduct} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700"><Plus className="h-4 w-4" /> Publish product</button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {products.map((product) => (
                  <div key={product.id} className="overflow-hidden rounded-xl border border-slate-200">
                    {product.image ? <img src={product.image} alt={product.name} className="h-40 w-full object-cover" /> : <div className="h-40 w-full bg-slate-200" />}
                    <div className="flex items-start justify-between gap-4 p-4">
                      ...
                      <button onClick={() => { const updated = products.filter((item) => item.id !== product.id); setProducts(updated); persist('adminProducts', updated); }} className="rounded-md p-2 text-red-600 transition hover:bg-red-50"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="space-y-6">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                  <input value={newGallery.src} onChange={(event) => setNewGallery((old) => ({ ...old, src: event.target.value }))} placeholder="Image URL" className={inputClass} />
                  <input value={newGallery.alt} onChange={(event) => setNewGallery((old) => ({ ...old, alt: event.target.value }))} placeholder="Caption" className={inputClass} />
                  <input value={newGallery.category} onChange={(event) => setNewGallery((old) => ({ ...old, category: event.target.value }))} placeholder="Category" className={inputClass} />
                </div>
                <button onClick={addGalleryItem} className="mt-3 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700"><Plus className="h-4 w-4" /> Add gallery photo</button>
              </div>
                ...
                {galleryItems.map((item) => (
                  <div key={item.id} className="overflow-hidden rounded-xl border border-slate-200">
                    ...
                    <div className="flex justify-between gap-4 p-4">
                      <div><p className="font-semibold text-slate-900">{item.alt}</p><p className="text-sm text-slate-500">{item.category}</p></div>
                      <button onClick={() => { const updated = galleryItems.filter((entry) => entry.id !== item.id); setGalleryItems(updated); persist('adminGalleryImages', updated); }} className="rounded-md p-2 text-red-600 transition hover:bg-red-50"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="space-y-6">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <div className="space-y-3">
                  <input value={newAnnouncement.title} onChange={(event) => setNewAnnouncement((old) => ({ ...old, title: event.target.value }))} placeholder="Announcement title" className={inputClass} />
                  <textarea value={newAnnouncement.content} onChange={(event) => setNewAnnouncement((old) => ({ ...old, content: event.target.value }))} placeholder="Announcement message" rows={4} className={inputClass} />
                  <select value={newAnnouncement.priority} onChange={(event) => setNewAnnouncement((old) => ({ ...old, priority: event.target.value as 'normal' | 'high' }))} className={inputClass}>
                    <option value="normal">Normal priority</option>
                    <option value="high">High priority</option>
                  </select>
                </div>
                <button onClick={publishAnnouncement} className="mt-3 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700"><Send className="h-4 w-4" /> Publish update</button>
              </div>
              <div className="space-y-3">
                {announcements.map((item) => (
                  <div key={item.id} className="flex items-start justify-between gap-4 rounded-xl border border-slate-200 p-4">
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-semibold text-slate-900">{item.title}</p>
                        {item.priority === 'high' && <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs font-semibold text-red-700">High</span>}
                      </div>
                      <p className="mt-1 text-sm text-slate-600">{item.content}</p>
                    </div>
                    <button onClick={() => { const updated = announcements.filter((entry) => entry.id !== item.id); setAnnouncements(updated); persist('siteAnnouncements', updated); }} className="rounded-md p-2 text-red-600 transition hover:bg-red-50"><Trash2 className="h-4 w-4" /></button>
                  </div>
                ))}
              </div>
            </section>
              ...
              <div className="grid grid-cols-1 gap-5 lg:grid-cols-[260px_1fr]">
                <div className="h-[430px] overflow-y-auto rounded-xl border border-slate-200 p-3">
                  {users.map((user) => {
                    ...
                    return (
                      <button key={user.email} onClick={() => setSelectedEmail(user.email)} className={`mb-2 w-full rounded-lg p-3 text-left transition ${isSelected ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'}`}>
                ...
                <div className="flex h-[430px] flex-col rounded-xl border border-slate-200 p-4">
                  <div className="mb-4 flex items-center gap-2 text-sm text-slate-700"><Users className="h-4 w-4" />{selectedUser ? `${selectedUser.name} (${selectedUser.email})` : 'Select a user'}</div>
                  <div className="flex-1 space-y-2 overflow-y-auto rounded-lg bg-slate-50 p-3">
                    {conversation.map((item) => (
                      <motion.div key={item.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={`max-w-[82%] rounded-lg px-3 py-2 text-sm ${item.sender === 'admin' ? 'ml-auto bg-blue-600 text-white' : 'border border-slate-200 bg-white text-slate-700'}`}>
                        <p>{item.message}</p>
                      </motion.div>
                    ))}
                  </div>
                  <div className="mt-3 flex gap-2">
                    <input value={newChatMessage} onChange={(event) => setNewChatMessage(event.target.value)} className={inputClass} placeholder="Reply to customer" />
                    <button onClick={sendAdminMessage} className="rounded-lg bg-blue-600 px-4 text-sm font-semibold text-white hover:bg-blue-700">Send</button>
                  </div>
                </div>
              </div>
            </section>
